...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long end(long transNum) {
        ...
        long prevLSN = transactionEntry.lastLSN;
        if(transactionEntry.transaction.getStatus() == Transaction.Status.COMPLETE || transactionEntry.transaction.getStatus() == Transaction.Status.COMMITTING){
        ...
        prevLSN = helper(record);
        transactionEntry.transaction.setStatus(Transaction.Status.COMPLETE);
        transactionTable.remove(transNum);
        LogRecord endRecord = new EndTransactionLogRecord(transNum, prevLSN);
        logManager.appendToLog(endRecord);
        if(record.getPrevLSN().isPresent()) return record.getPrevLSN().get();
        return -1L;
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        if(before.length > BufferManager.EFFECTIVE_PAGE_SIZE/2){
            LogRecord record1 = new UpdatePageLogRecord(transNum, pageNum, prevLSN, pageOffset, before, null);
            long prevLSN1 = logManager.appendToLog(record1);
            LogRecord record2 = new UpdatePageLogRecord(transNum, pageNum, prevLSN1, pageOffset, null, after);
            LSN = logManager.appendToLog(record2);
        } else {
    ...
    public void rollbackToSavepoint(long transNum, String name) {
        ...
        long prevLSN = transactionEntry.lastLSN;
        LogRecord record = logManager.fetchLogRecord(prevLSN);
        while(record.getPrevLSN().isPresent() && record.getLSN() != LSN){
            prevLSN = record.getPrevLSN().get();
            if(record.type == LogType.ABORT_TRANSACTION){
                record = logManager.fetchLogRecord(prevLSN);
                continue;
            }
            Pair<LogRecord, Boolean> pair = record.undo(prevLSN);
            LogRecord newRecord = pair.getFirst();
            Boolean flush = pair.getSecond();
            if(newRecord.getType() == LogType.UNDO_UPDATE_PAGE){
                dirtyPageTable.putIfAbsent(record.getPageNum().get(), prevLSN);
            } else if(newRecord.getType() == LogType.UNDO_ALLOC_PAGE){
                dirtyPageTable.remove(record.getPageNum());
            }
            if(flush){
    ...
    public void checkpoint() {
        ...
        Set entrySet = dirtyPageTable.entrySet();
        Iterator iter = entrySet.iterator();
        while(iter.hasNext()){
            Map.Entry entry = (Map.Entry) iter.next();
            dpt.put((long)entry.getKey(),(long)entry.getValue());
        }
        entrySet = transactionTable.entrySet();
        iter = entrySet.iterator();
        while(iter.hasNext()){
            Map.Entry entry = (Map.Entry) iter.next();
            long key = (long) entry.getKey();
            TransactionTableEntry tEntry = transactionTable.get(key);
            long prevLSN = tEntry.lastLSN;
            Pair<Transaction.Status, Long> pair = new Pair(tEntry.transaction.getStatus(),prevLSN);
            txnTable.put(tEntry.transaction.getTransNum(),pair);
            Set<Long> tPages = tEntry.touchedPages;
            List<Long> toPages = new ArrayList<Long>(tPages.size());
            for (Long l : tPages){
                toPages.add(l);
            }
            touchedPages.put(tEntry.transaction.getTransNum(),toPages);
        }
    ...
    public long helper(LogRecord record){
        long prevLSN = record.getLSN();
